import React from 'react'
import { useSelector } from 'react-redux';

const BlogP = () => {
   const ar = useSelector(state => state.langs.value)
   return (
      <div dir={ar ? "rtl" : "ltr"} className="container py-6 grid md:grid-cols-8">
         <aside className={`col-span-8 md:col-span-2 py-4 md:p-4 ${ar ? "md:border-l" : "md:border-r"} md:border-gray-200`}>
            <h3 className="text-2xl mb-4">{ar ? "المدونات" : "Blog"}</h3>
            <div className="bg-gray-50 space-y-4 p-4 rounded shadow border">
               <h4 className="text-xl mb-2">{ar ? "أهم المدونات" : "Top Blogs"}</h4>
               <div role="status" className="w-[90%] animate-pulse">
                  <div className="h-2.5 bg-gray-200 rounded-full" />
               </div>
               <div role="status" className="w-[50%] animate-pulse">
                  <div className="h-2.5 bg-gray-200 rounded-full" />
               </div>
               <div role="status" className="w-[70%] animate-pulse">
                  <div className="h-2.5 bg-gray-200 rounded-full" />
               </div>
            </div>
            <div className="bg-gray-50 space-y-4 p-4 rounded shadow border mt-6">
               <h4 className="text-xl mb-2">{ar ? "الفئات" : "Categories"}</h4>
               <div role="status" class="w-[50%] animate-pulse">
                  <div className="h-2 bg-gray-200 rounded-full" />
               </div>
               <div role="status" class="w-[60%] animate-pulse">
                  <div className="h-2 bg-gray-200 rounded-full" />
               </div>
               <div role="status" class="w-[40%] animate-pulse">
                  <div className="h-2 bg-gray-200 rounded-full" />
               </div>
            </div>
         </aside>
         <div className="h-fit col-span-8 md:col-span-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:px-6 gap-4">
            <div
               className="bg-gray-50 rounded-lg border shadow-md"
            >
               <div role="status" className="space-y-8 animate-pulse md:space-y-0 md:space-x-8 md:flex md:items-center">
                  <div className="flex justify-center items-center w-full h-52 bg-gray-300 rounded">
                     <svg className="w-12 h-12 text-gray-200" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" fill="currentColor" viewBox="0 0 640 512"><path d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z" /></svg>
                  </div>
               </div>
               <div className="p-5">
                  <div role="status" className="max-w-sm mb-4 animate-pulse">
                     <div className="h-2.5 bg-gray-200 rounded-full w-48 mb-4" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[360px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[330px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[300px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[360px]" />
                     <span className="sr-only">Loading...</span>
                  </div>
                  <a className="mt-auto w-40 py-4 inline-flex items-center px-3 bg-primary-200 rounded-lg">
                     <div role="status" className="w-[50%] mx-auto animate-pulse">
                        <div className="h-2.5 bg-primary-300 rounded-full" />
                     </div>
                  </a>
               </div>
            </div>
            <div
               className="bg-gray-50 rounded-lg border shadow-md"
            >
               <div role="status" className="space-y-8 animate-pulse md:space-y-0 md:space-x-8 md:flex md:items-center">
                  <div className="flex justify-center items-center w-full h-52 bg-gray-300 rounded">
                     <svg className="w-12 h-12 text-gray-200" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" fill="currentColor" viewBox="0 0 640 512"><path d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z" /></svg>
                  </div>
               </div>
               <div className="p-5">
                  <div role="status" className="max-w-sm mb-4 animate-pulse">
                     <div className="h-2.5 bg-gray-200 rounded-full w-48 mb-4" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[360px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[330px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[300px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[360px]" />
                     <span className="sr-only">Loading...</span>
                  </div>
                  <a className="mt-auto w-40 py-4 inline-flex items-center px-3 bg-primary-200 rounded-lg">
                     <div role="status" className="w-[50%] mx-auto animate-pulse">
                        <div className="h-2.5 bg-primary-300 rounded-full" />
                     </div>
                  </a>
               </div>
            </div>
            <div
               className="bg-gray-50 rounded-lg border shadow-md"
            >
               <div role="status" className="space-y-8 animate-pulse md:space-y-0 md:space-x-8 md:flex md:items-center">
                  <div className="flex justify-center items-center w-full h-52 bg-gray-300 rounded">
                     <svg className="w-12 h-12 text-gray-200" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" fill="currentColor" viewBox="0 0 640 512"><path d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z" /></svg>
                  </div>
               </div>
               <div className="p-5">
                  <div role="status" className="max-w-sm mb-4 animate-pulse">
                     <div className="h-2.5 bg-gray-200 rounded-full w-48 mb-4" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[360px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[330px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[300px] mb-2.5" />
                     <div className="h-2 bg-gray-200 rounded-full max-w-[360px]" />
                     <span className="sr-only">Loading...</span>
                  </div>
                  <a className="mt-auto w-40 py-4 inline-flex items-center px-3 bg-primary-200 rounded-lg">
                     <div role="status" className="w-[50%] mx-auto animate-pulse">
                        <div className="h-2.5 bg-primary-300 rounded-full" />
                     </div>
                  </a>
               </div>
            </div>

         </div>
      </div>
   )
}

export default BlogP