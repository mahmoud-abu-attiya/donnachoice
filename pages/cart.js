...
const Cart = () => {
   ...
   return (
      <div dir={ar ? "rtl" : "ltr"} className="container">
         ...
         {ar ? <h2 className='text-3xl text-gray-700'>
         عناصر عربة التسوق/
            <span className="text-sm">
            أضف جديدًا أو استخدم عنوان التسليم الحالي.
            </span>
         </h2> : <h2 className='text-3xl text-gray-700'>
            Delivery Details /
            <span className="text-sm">
         ...
         <div className='mt-8'>
            <div className="hidden lg:block mb-4 border-gray-200 bg-primary-300 text-primary-100  rounded-lg overflow-hidden">
               <ul className="flex flex-wrap -mb-px text-sm font-medium text-center">
                  <li className="grow bg-primary-100 text-primary-300">
                     <div className={`flex gap-4 justify-center items-center py-4 ${ar ? "border-l" : "border-r"} border-primary-100`}>
            ...
            {products.length == 0 ? (
               <div className='text-2xl capitalize text-center col-span-4'>
                  ...
                  {ar ? "لا توجد منتجات في عربة التسوق الخاصة بك" :"There no products in your cart."} <br />
                  <Link href={"/products"}>
               ...
               <div className='grid grid-cols-1 lg:grid-cols-8 gap-4 mb-8'>
                  <div className="col-span-8 lg:col-span-6 overflow-x-auto relative h-fit rounded-lg border">
                     <table className={`w-full text-sm text-left text-gray-500 ${ar && "text-right"}`}>
                                    ...
                                    return (
                                       <tr key={product.id} className="bg-white border-b">
                                          ...
                                          <th className="py-4 px-6 font-medium text-gray-900 whitespace-nowrap">
                                             ...
                                             {ar ? product.product.name_ar : product.product.name} ({product.name})
                                          </th>
                                          ...
                                          <td className="py-4 px-6 product-amount">
                                             ...
                                             <div className="flex items-center gap-3">
                                                <button onClick={() => setproCount(proCount - 1)} className="inline-flex items-center p-1 text-sm font-medium text-gray-500 bg-white rounded-full border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200" type="button">
                                                ...
                                                <div>
                                                   <input defaultValue={proCount} type="number" min={1} id="first_product" className="bg-gray-50 w-14 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1" placeholder={product.amount || product.quantity || 1} required />
                                                </div>
                                                <button onClick={() => setproCount(proCount + 1)} className="inline-flex items-center p-1 text-sm font-medium text-gray-500 bg-white rounded-full border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200" type="button">
                                                   <span className="sr-only">Quantity button</span>
                  ...
                  <div className="col-span-8 lg:col-span-2 flex flex-col gap-4">
                     <div className="bg-gray-50 p-4 border rounded-md">
                        ...
                        <div className='capitalize'>{ar ? "مجموع العناصر :" :"total items: "}<span className='text-xl font-bold'> {totalAmount}</span></div>
                        <div className='capitalize'>{ar ? "السعر الإجمالي (ريال قطري)" : "total price (QR) : "}<span className='text-xl font-bold'> {totalPrice}</span></div>
                     </div>
                     <button className='w-full bg-primary-100 text-white rounded-md py-4'>{ar ? "التالي" :"Next"} <i className={`fas ${ar ? "fa-arrow-left" : "fa-arrow-right"}`}></i> {ar ? "توصيل" :"Delivery"}</button>
                  </div>
               </div>
            )}
         </div>
